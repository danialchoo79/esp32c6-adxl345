...
#define ACC (0x53)//(0xA7>>1) I2C Address
#define A_TO_READ (6)
...
void initAcc(uint8_t scl_pin, uint8_t sda_pin) {
	...
	writeTo(ACC, 0x2D, 1 << 3); // 100 Measure Mode
	writeTo(ACC, 0x31, 0x0B); // 1011 11 +- 16g Range
	writeTo(ACC, 0x2C, 0x09); // 1001 Rate Code 50Hz DR, 25Hz BW
}
...
void getAccelerometerData(int *result) {
	...
	result[0] = (((int) buff[1]) << 8) | buff[0]; // combining two bytes for 16 bits representation
